...
// @version      0.2
// @description  steam搜索输入框添加搜索历史记录框,游戏商店浏览足迹
// @author       wsz987
...
`)
    initHistory()
    $('.apphub_AppName').text()? GM_setValue('footName',$('.apphub_AppName').text()): false
})();
...
function initHistory(){
    ...
    let formCtx=$("#store_search_link").parents('form').context.referrer
    ...
    $("#store_nav_search_term, input#term") ? $(".searchbox, .searchbar_left").append(table_DOM):''
    // DOM 动态style
    $("#store_nav_search_term, input#term").click(el=>{
        $("#"+el.target.id+" ~ .history-table").css('width',el.target.id==='store_nav_search_term'?
                                                    $('.searchbox')[0].offsetWidth-33 : $("#"+el.target.id)[0].offsetWidth+'px')
        $("#"+el.target.id+" ~ .history-table").css('display','block')
    }).blur(el=>{
        setTimeout(function(){
            $("#"+el.target.id+" ~ .history-table").css('display','none')
        },250)
    });
    // 点击搜索监听
    $('#store_search_link,.searchbar_left button').click(()=>{
        setHistoryData()
    })
    // 回车搜索监听
    $('#store_nav_search_term,input#term').keydown(function(event){
        if(event.keyCode==13){
            setHistoryData()
        }
    })
    // 全局事件委托监测动态DOM
    $('body').on("click",'#term_options > ul > li',()=>{
        setTimeout(function(){
            setHistoryData()
        },250)
    })
...
function setHistoryData(){
    let input_val=$("#store_nav_search_term")[0].value.toString().trim() || $("input#term")[0].value.toString().trim()
    if(input_val==='') return
    let oldVal=GM_getValue('historyData')
    oldVal.unshift(input_val)
    oldVal=Array.from(new Set(oldVal))
    oldVal.length > window.HistoryNum ? oldVal.pop():console.log('historyData')
